...
### Introduction
I always owned a lot of hard drives: I don't know why, I always used and still use to look for space to save my data. In the years, I started using disks, then I assembled a HP Proliant to be a Synology Based System - I don't want to go the cloud because I'm stupid - and... in the last week, I decided to make order in a huge amount of files. The first thing you have to do when you are handling terabytes and terabytes of __both__ well-ordered __and__ _no-ordered-at-all_ data is literaly pray that someone else, like a magician, or druid comes to you with a magic wand and fixes all the mess for free, in a way you do not know but you'll like it.
...
In the next paragraphs, I will talk about how you can setup your envinroment locally, without losing too much time in configuration and start building what you need to explore your file systems.
...
Elasticsearch is a distributed, RESTful search and analytics engine capable of solving different _scenarios_: what do I mean? That it could be use to centrally store your data, so you can discover the expected and uncover the unexpected. Perfect for my original purpose: make order in my file systems. How can you install elastisearch in a portable way? It's really simple! Open a shell and do the following:
...
In any case[^kt], simply run Kibana with ```./bin/kibana```. And... again, congratulations, you are done! If you go now to the [http://localhost:5601](http://localhost:5601) you should see the Kibana web console. Now that you have a running Elasticsearch core and a Kibana web console to look at your stack - cool, isn't it? - let's move to the most important part: how to generate data starting from the file systems to analyze them.
...
In any case, I created a [repository](https://github.com/made2591/python-multi-walker) to share my snippet to create my data: using the method ```os.stats``` - more info [here](https://docs.python.org/2/library/stat.html) - of Python standard lib, I created script that log for me to a JSON file named ```files.json``` with info about each file in my file system that follows this schema:
...
The stats for each file are extened with, of course, file name and extensions - even if filename (complete path) is sufficient, I wanted also a separated field with ext to create filters more easly later. For what does it concern the Python script, you can customize the starting directory, the number of processes you want to run the walk (if you have a distributed file systems) and the output file path.
...
A table should appear with all information and inferred types for each field about the data you load previously using the bulk api - more details [here](https://www.elastic.co/guide/en/elasticsearch/reference/current/docs-bulk.html).
